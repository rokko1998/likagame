...
|---|---|---|
| `MVP-0 / playable spike` | как можно быстрее проверить запуск, touch, масштабирование и основной math→world loop | Telegram/browser host, одна тема на временных ассетах, E01 и один encounter целевого факта, локально подготовленные голосовые fallback-реплики, локальный checkpoint и telemetry |
...
| `Post-demo` | развивать повторяемость и долгосрочную метаигру | модульная сборка приключений из пула комнат, расширенный контент, мета-база и серверно-авторитетная экономика |
...
- Реальная загрузка сборки внутри Telegram Mini App и в standalone browser mode.
- Первый checkpoint: один `allocate_equal` encounter и отдельный технический smoke системной цифровой клавиатуры на временных ассетах. Smoke не является Q46, не создаёт mastery/loot и не нарушает учебную последовательность. До завершения всего Этапа 0 добавляется один `array_transform` encounter, чтобы проверить второй controller и раздельные `math_status/scene_status`.
- Canvas-мультсцена с говорящим персонажем без видимого текста реплик и DOM-поле с `inputmode="numeric"`.
...
- Имитация задержки/ошибки LLM и TTS с немедленным fallback.
- Ручная проверка минимум в Telegram iOS, Telegram Android на слабом/среднем устройстве и Telegram Desktop.

Результат: подтверждено, что выбранный web stack, ввод и Telegram-контейнер годятся для игры. До этого результата не производится полный набор фонов, персонажей и loot-ассетов.
...
11. TTS и упрощённое движение рта входят в демо.
...
28. Стек демо — TypeScript, React, Phaser, Vite, Node/Fastify и PostgreSQL; параллельная реализация на другом клиентском движке не ведётся.
...
- конкретный TTS-голос для каждого персонажа;
...
- перечень всех авторских fallback-реплик;
...
### 31.1. Первый playable checkpoint

До производства полного контента реализуется один сквозной путь:

```text
Telegram/Browser launch
→ fixed RunPlan
→ короткая голосовая мультсцена без видимого текста реплик
→ E01 allocate_equal с tap/drag
→ commit и ValidationResult
→ реакция мира + fallback-реплика
...
- React/Vite shell и одна Phaser scene;
- `BrowserHost`, `TestHost` и минимальный `TelegramHost`;
- чистый `game-core` reducer, один игровой `EncounterDefinition`, один validator, отдельный недоменный input-smoke и fixed `ContentBundle`;
- реальные JSON Schema-файлы в `packages/contracts/schemas` и один связный fixture `E01 → SceneDefinition → AdventureBlock → fixed RunPlan → dialogue → asset manifest`; prose-пример без проходящего schema test checkpoint не закрывает;
- mouse/touch parity, `pointercancel`, safe area, portrait screen и background pause;
- авторские реплики в content contract, локально подготовленная озвучка, fake lip sync и повтор тапом по персонажу без видимых диалоговых карточек;
- `IndexedDB` crash cache и выгрузка обезличенного telemetry trace;
- небольшой curated subset Kenney с заполненным asset manifest;
- Vitest reducer/validator tests и один Playwright happy path + reload path.

Этот checkpoint считается пройденным только внутри реального Telegram iOS, Android и Desktop, а также в обычном браузере.

### 31.2. Следом, не раньше playable checkpoint

1. Добавить E03 `array_transform`, чтобы проверить второй тип управления и различие `math_status/scene_status`.
2. Подключить Fastify Telegram auth, PostgreSQL checkpoint и idempotent command API.
3. Собрать линейный каркас Space vertical slice из трёх глав и всех обязательных encounters.
4. Добавить полную H0–H5, TTS, затем активировать S1/G1/S2 поверх уже проходящего каркаса.
5. Только после устойчивого основного loop подключать loot/pity/R1, динамические LLM-реплики и второй Theme Pack.

До этого момента не реализуются pool generator, произвольная генерация комнат, мета-база и production economy. Их контракты уже предусмотрены, но они не должны увеличивать время до первого наблюдаемого детского теста.
